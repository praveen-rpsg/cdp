{{ config(materialized='table', schema='silver_reverse_etl') }}

WITH profiles AS (
    SELECT * FROM {{ ref('unified_profiles') }}
),

txn_summary AS (
    SELECT * FROM {{ ref('customer_transaction_summary') }}
),
...
channel_summary AS (
    ...
    FROM {{ ref('customer_channel_summary') }}
