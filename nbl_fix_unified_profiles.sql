-- =============================================================================
-- NBL: Fix unified_profiles — POS-first identity resolution
-- =============================================================================
-- Root cause: int_identity_resolved was CIH-first, dumping all 2.7M CIH rows
-- into unified_profiles. Correct scope is transaction customers (~28K) enriched
-- by CIH where available.
--
-- Re-runs (in dependency order):
--   0c. nb_silver.int_identity_resolved     (POS-first rebuild)
--   1a. nb_silver_identity.unified_profiles
--   1b. nb_silver_identity.identity_edges
--   1c. nb_silver_identity.identity_graph_summary
--   2d. nb_silver_gold.customer_transaction_summary  (joins unified_profiles)
--   3.  nb_silver_reverse_etl.customer_behavioral_attributes
--
-- Skipped (correct, no dependency on unified_profiles):
--   0a. stg_bill_summary
--   0b. int_identity_spine
--   2a. customer_channel_summary
--   2b. customer_product_summary
--   2c. daily_store_sales
-- =============================================================================

BEGIN;

-- ---------------------------------------------------------------------------
-- 0c. int_identity_resolved — POS-first
--     Base = distinct transaction mobiles; LEFT JOIN CIH for enrichment.
--     Every row has has_transactions = TRUE.
-- ---------------------------------------------------------------------------
...
DO $$ BEGIN RAISE NOTICE 'int_identity_resolved: % rows', (SELECT COUNT(*) FROM nb_silver.int_identity_resolved); END $$;
...
DO $$ BEGIN RAISE NOTICE 'unified_profiles: % rows', (SELECT COUNT(*) FROM nb_silver_identity.unified_profiles); END $$;
...
DO $$ BEGIN RAISE NOTICE 'identity_edges: % rows', (SELECT COUNT(*) FROM nb_silver_identity.identity_edges); END $$;

-- ---------------------------------------------------------------------------
-- 1c. identity_graph_summary
-- ---------------------------------------------------------------------------
...
DO $$ BEGIN RAISE NOTICE 'identity_graph_summary: % rows', (SELECT COUNT(*) FROM nb_silver_identity.identity_graph_summary); END $$;

-- ---------------------------------------------------------------------------
-- 2d. customer_transaction_summary (re-run: joins unified_profiles)
-- ---------------------------------------------------------------------------
...
DO $$ BEGIN RAISE NOTICE 'customer_transaction_summary: % rows', (SELECT COUNT(*) FROM nb_silver_gold.customer_transaction_summary); END $$;

-- ---------------------------------------------------------------------------
-- 3. customer_behavioral_attributes
-- ---------------------------------------------------------------------------
...
DO $$ BEGIN RAISE NOTICE 'customer_behavioral_attributes: % rows', (SELECT COUNT(*) FROM nb_silver_reverse_etl.customer_behavioral_attributes); END $$;
