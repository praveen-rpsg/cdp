...
ATTRIBUTE_CATALOG: list[AttributeDefinition] = [
    ...
    AttributeDefinition(
        key="geo.city",
        label="City (Registered Home City)",
        category=AttributeCategory.GEOGRAPHIC,
        data_type=AttributeDataType.STRING,
        description=(
            "Customer's registered home city from their profile — NOT where they transact. "
            "Use bt.city_desc instead when the user asks about city in a query context "
            "(e.g. 'customers in Kolkata', 'Mumbai shoppers'). "
            "geo.city is only relevant for demographic city-of-residence filters."
        ),
        operators=STRING_OPS,
    ...
    AttributeDefinition(
        ...
        operators=STRING_OPS,
        example_values=["SOUTH", "EAST", "WEST", "NORTH", "PARK"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="raw_location_master",
    ),
    AttributeDefinition(
    ...
    # =========================================================================
    # BILL TRANSACTION (BT) ATTRIBUTES  (table: s_fact_bill_transactions)
    # Line-item level — compiled as EXISTS subqueries in PgCompiler
    # =========================================================================
    AttributeDefinition(
        key="bt.bill_date",
        label="BT Bill Date",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.DATE,
        description="Date of the bill transaction (use to scope BT queries by time range)",
        operators=DATE_OPS,
        example_values=["2026-01-15", "2026-02-01"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.brand_name",
        label="BT Brand Name",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Brand of the product purchased in the transaction",
        operators=STRING_OPS,
        example_values=["AMUL", "BRITANNIA", "FORTUNE", "SMART CHOICE", "SURF EXCEL"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.segment_desc",
        label="BT Product Segment",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Highest product hierarchy level (segment) of the purchased article",
        operators=STRING_OPS,
        example_values=["FOOD", "NON FOOD GROCERY", "NON TRADE", "GM", "FASHION CB", "HI TECH"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.family_desc",
        label="BT Product Family",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Product family level in the merchandise hierarchy",
        operators=STRING_OPS,
        example_values=["PROCESSED FOOD", "HEALTH & BEAUTY", "MASALA SPICES & HERB", "FRESH VEGETABLE", "CHILLED & FROZEN", "HOUSE HOLD NEED"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.class_desc",
        label="BT Product Class",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Product class level in the merchandise hierarchy",
        operators=STRING_OPS,
        example_values=["MASALA & SPICE", "BISCUIT", "FRESH VEG", "DAIRY", "UNSPECIFIED CATEGORY", "CONFECTIONARY"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.brick_desc",
        label="BT Product Brick",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description=(
            "Most granular product CATEGORY in Spencer's hierarchy (brick level). "
            "Use this for product types/categories that are NOT specific branded SKUs. "
            "Examples of brick-level categories: BATH MUGS, WHISKY, PULSES, WHOLE SPICE, "
            "BROOMS & BRUSHES, HAIR OIL, FLOOR CLEANER, PLASTIC CONTAINERS. "
            "Do NOT use bt.article_desc for category-level queries."
        ),
        operators=STRING_OPS,
        example_values=[
            "WHOLE SPICE", "STRAIGHT SPICE PWDR", "WHISKY", "LOCAL NAMKEEN",
            "PULSES", "BATH MUGS", "BROOMS & BRUSHES", "HAIR OIL",
            "FLOOR CLEANER", "PLASTIC CONTAINERS", "PACKAGED DRINKING WATER",
            "REFINED OIL", "BISCUIT COOKIES", "CHOCOLATE BAR",
        ],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.article_desc",
        label="BT Article / SKU",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description=(
            "Specific branded product / SKU name purchased (NOT a category). "
            "Use ONLY when the user mentions a specific brand+product (e.g. 'Amul Taaza 500ml', "
            "'Surf Excel 1kg'). For product categories like 'Bath Mugs', 'Whisky', 'Pulses' "
            "use bt.brick_desc instead."
        ),
        operators=STRING_OPS,
        example_values=["AMUL TAAZA 500ML", "TATA SALT 1KG", "SURF EXCEL 1KG", "FORTUNE SUNLITE 1LTR", "MAGGI NOODLES 70G"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.article",
        label="BT Article Code",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Article/SKU code of the purchased product",
        operators=STRING_OPS,
        example_values=["100012345", "200067890"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.manufacturer_name",
        label="BT Manufacturer",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Manufacturer of the purchased article",
        operators=STRING_OPS,
        example_values=["HINDUSTAN UNILEVER", "P&G", "DABUR", "AMUL", "ITC LIMITED"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        ...
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Store/plant code where the transaction occurred",
        operators=STRING_OPS,
        example_values=["SP001", "SP045"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ...
    AttributeDefinition(
        ...
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Store description where the transaction occurred",
        operators=STRING_OPS,
        example_values=["Spencer's Park Street", "Spencer's Indiranagar"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.store_format",
        label="BT Store Format",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Store format (Hyper, Daily, Express) of the transaction store",
        operators=STRING_OPS,
        example_values=["Hyper", "Daily", "Express"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ...
    AttributeDefinition(
        key="bt.city_desc",
        label="BT Transaction City",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description=(
            "PREFERRED city attribute. City where the bill transaction physically occurred. "
            "Always use this when the user says 'in <city>', '<city> customers', or '<city> shoppers'. "
            "Examples: 'customers in Kolkata' → bt.city_desc equals 'Kolkata'."
        ),
        operators=STRING_OPS,
        example_values=["Kolkata", "Bengaluru", "Hyderabad"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.region_desc",
        label="BT Transaction Region",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="State where the transaction occurred (mapped as region in source)",
        operators=STRING_OPS,
        example_values=["West Bengal", "Uttar Pradesh", "Karnataka", "Gujarat"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.delivery_channel",
        label="BT Delivery Channel",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Whether the transaction was Online or Store (derived from till_no)",
        operators=STRING_OPS,
        example_values=["Store", "Online"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.gross_sale_value",
        label="BT Gross Sale Value",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.FLOAT,
        description="Gross sale value of the line item",
        operators=NUMERIC_OPS,
        example_values=[50.0, 500.0, 5000.0],
        applicable_brands=["spencers", "natures_basket"],
        unit="INR",
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.total_discount",
        label="BT Line Discount",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.FLOAT,
        description="Discount amount on the line item",
        operators=NUMERIC_OPS,
        example_values=[0.0, 25.0, 100.0],
        applicable_brands=["spencers", "natures_basket"],
        unit="INR",
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.total_mrp_value",
        label="BT Total MRP Value",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.FLOAT,
        description="Total MRP value of the line item",
        operators=NUMERIC_OPS,
        example_values=[100.0, 500.0, 2000.0],
        applicable_brands=["spencers", "natures_basket"],
        unit="INR",
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.billed_qty",
        label="BT Billed Quantity",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.FLOAT,
        description="Quantity of the article billed",
        operators=NUMERIC_OPS,
        example_values=[1.0, 2.0, 5.0, 10.0],
        applicable_brands=["spencers", "natures_basket"],
        unit="qty",
        source_table="s_fact_bill_transactions",
    ),
    AttributeDefinition(
        key="bt.sales_return",
        label="BT Sales Return",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.BOOLEAN,
        description="Whether this line item is a sales return",
        operators=BOOLEAN_OPS,
        example_values=[True, False],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ...
    AttributeDefinition(
        key="bt.weekend_flag",
        label="BT Weekend Transaction",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.BOOLEAN,
        description="Whether the transaction occurred on a weekend (Saturday or Sunday)",
        operators=BOOLEAN_OPS,
        example_values=[True, False],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ...
    AttributeDefinition(
        key="bt.wednesday_flag",
        label="BT Wednesday Transaction",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.BOOLEAN,
        description="Whether the transaction occurred on a Wednesday (Spencer's promo day)",
        operators=BOOLEAN_OPS,
        example_values=[True, False],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ...
    AttributeDefinition(
        key="bt.promo_indicator",
        label="BT Promo Applied",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Promotion indicator ('P' if promo was applied to this line item)",
        operators=STRING_OPS,
        example_values=["P", ""],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ...
    AttributeDefinition(
        key="bt.day_of_week",
        label="BT Day of Week",
        category=AttributeCategory.BILL_TRANSACTION,
        data_type=AttributeDataType.STRING,
        description="Day of the week for the transaction (Monday, Tuesday, etc.)",
        operators=STRING_OPS,
        example_values=["Monday", "Wednesday", "Saturday", "Sunday"],
        applicable_brands=["spencers", "natures_basket"],
        source_table="s_fact_bill_transactions",
    ...
    # =========================================================================
    # CROSS-BRAND ATTRIBUTES  (table: corporate_cih.silver_corp_customer_attributes)
    # Applicable only to brand_code="corporate".
    # spn.* keys → corp.spn_* columns  (Spencers behavioral data)
    # nbl.* keys → corp.nbl_* columns  (Nature's Basket behavioral data)
    # corp.* keys → corp.*  (RPSG cross-brand metrics / identity)
    # =========================================================================

    # ── Corporate / RPSG identity ──────────────────────────────────────────────
    AttributeDefinition(
        key="corp.rpsg_brand_presence",
        label="Brand Presence",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description=(
            "Brands this customer is registered with across RPSG (stored as Python list repr, "
            "e.g. \"['Spencer' 'NBL']\"). Use 'contains' with a single brand name like 'Spencer' or 'NBL'. "
            "NEVER use 'equals' — for brand membership always prefer corp.is_spencers_customer or corp.is_nbl_customer."
        ),
        operators=["contains", "not_contains", "exists", "not_exists"],
        example_values=["Spencer", "NBL", "CESC"],
        applicable_brands=["corporate"],
        source_table="silver_corp_customer_attributes",
    ),
    AttributeDefinition(
        key="corp.rpsg_ftd",
        label="RPSG First Transaction Date",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.DATE,
        description="Earliest ever transaction date across all RPSG brands",
        operators=DATE_OPS,
        example_values=["2020-01-15", "2022-06-01"],
        applicable_brands=["corporate"],
        source_table="silver_corp_customer_attributes",
    ),
    AttributeDefinition(
        key="corp.rpsg_ltd",
        label="RPSG Last Transaction Date",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.DATE,
        description="Most recent transaction date across all RPSG brands",
        operators=DATE_OPS,
        example_values=["2025-12-20", "2026-01-05"],
        applicable_brands=["corporate"],
        source_table="silver_corp_customer_attributes",
    ...
    AttributeDefinition(
        key="corp.rpsg_tenure_lifetime",
        label="RPSG Lifetime Tenure (Days)",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Customer tenure in days across all RPSG brands (first to last transaction)",
        operators=NUMERIC_OPS,
        example_values=[30, 365, 730, 1500],
        applicable_brands=["corporate"],
        unit="days",
        is_computed=True,
        source_table="silver_corp_customer_attributes",
    ...
    AttributeDefinition(
        key="corp.rpsg_recency_lifetime",
        label="RPSG Lifetime Recency (Days)",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Days since last transaction across all RPSG brands",
        operators=NUMERIC_OPS,
        example_values=[5, 30, 90, 365],
        applicable_brands=["corporate"],
        unit="days",
    ...
    AttributeDefinition(
        key="corp.is_spencers_customer",
        label="Is Spencers Customer",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.BOOLEAN,
        description="True if this customer has Spencers transactional data in the corporate view",
    ...
    AttributeDefinition(
        key="corp.is_nbl_customer",
        label="Is NBL Customer",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.BOOLEAN,
        description="True if this customer has Nature's Basket transactional data in the corporate view",
        operators=EXISTS_OPS,
        example_values=[True, False],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    # ── Spencers (SPN) lifecycle & core transactional ─────────────────────────
    AttributeDefinition(
        key="spn.lifecycle_stage",
        label="SPN Lifecycle Stage",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="Spencers lifecycle stage (Active / At Risk / Lapsed / Churned / Registered)",
        operators=STRING_OPS + EXISTS_OPS,
        example_values=["Active", "At Risk", "Lapsed", "Churned", "Registered"],
        applicable_brands=["corporate"],
        is_computed=True,
        source_table="silver_corp_customer_attributes",
    ...
    AttributeDefinition(
        key="spn.total_bills",
        label="SPN Total Bills",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Total number of Spencers bills for this customer",
        operators=NUMERIC_OPS + EXISTS_OPS,
        example_values=[1, 5, 20, 100],
        applicable_brands=["corporate"],
        unit="count",
        source_table="silver_corp_customer_attributes",
    ...
    AttributeDefinition(
        key="spn.total_spend",
        label="SPN Total Spend",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.FLOAT,
        description="Cumulative spend at Spencers across all transactions",
        operators=NUMERIC_OPS,
        example_values=[500.0, 5000.0, 50000.0],
        applicable_brands=["corporate"],
        unit="INR",
    ...
    AttributeDefinition(
        key="spn.spend_per_bill",
        label="SPN Spend per Bill",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.FLOAT,
        description="Average spend per bill at Spencers",
        operators=NUMERIC_OPS,
        example_values=[250.0, 800.0, 2000.0],
        applicable_brands=["corporate"],
        unit="INR",
    ...
    AttributeDefinition(
        key="spn.recency_days",
        label="SPN Recency (Days)",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Days since last Spencers transaction",
        operators=NUMERIC_OPS,
        example_values=[5, 30, 90, 365],
        applicable_brands=["corporate"],
        unit="days",
    ...
    AttributeDefinition(
        key="spn.last_bill_date",
        label="SPN Last Bill Date",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.DATE,
        description="Date of last Spencers transaction",
        operators=DATE_OPS,
        example_values=["2025-12-20", "2026-01-05"],
        applicable_brands=["corporate"],
        source_table="silver_corp_customer_attributes",
    ...
    AttributeDefinition(
        key="spn.l1_segment",
        label="SPN L1 Segment",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="Spencers value segment (HVHF / LVHF / HVLF / LVLF)",
        operators=STRING_OPS,
        example_values=["HVHF", "LVHF", "HVLF", "LVLF"],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    AttributeDefinition(
        key="spn.l2_segment",
        label="SPN L2 Segment",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="Spencers behavioral segment (STAR / LOYAL / Win Back / New / ACTIVE / Inactive / LAPSER / Deep Lapsed)",
        operators=STRING_OPS,
        example_values=["STAR", "LOYAL", "Win Back", "New", "ACTIVE", "Inactive", "LAPSER", "Deep Lapsed"],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    AttributeDefinition(
        ...
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="City from the customer's Spencers profile",
        operators=STRING_OPS,
        example_values=["Kolkata", "Hyderabad", "Bengaluru"],
        applicable_brands=["corporate"],
        source_table="silver_corp_customer_attributes",
    ),
    AttributeDefinition(
        key="spn.channel_presence",
        label="SPN Channel Presence",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="Spencers channel footprint: Online / Offline / Omni",
        operators=STRING_OPS,
        example_values=["Online", "Offline", "Omni"],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    AttributeDefinition(
        key="spn.spend_decile",
        label="SPN Spend Decile",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Spend decile within Spencers (1=lowest, 10=top)",
        operators=NUMERIC_OPS,
        example_values=[1, 3, 5, 7, 10],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    # ── Nature's Basket (NBL) lifecycle & core transactional ──────────────────
    AttributeDefinition(
        key="nbl.lifecycle_stage",
        label="NBL Lifecycle Stage",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="Nature's Basket lifecycle stage (Active / At Risk / Lapsed / Churned / Registered)",
        operators=STRING_OPS + EXISTS_OPS,
        example_values=["Active", "At Risk", "Lapsed", "Churned", "Registered"],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    AttributeDefinition(
        key="nbl.total_bills",
        label="NBL Total Bills",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Total number of Nature's Basket bills for this customer",
        operators=NUMERIC_OPS + EXISTS_OPS,
    ...
    AttributeDefinition(
        key="nbl.total_spend",
        label="NBL Total Spend",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.FLOAT,
        description="Cumulative spend at Nature's Basket",
        operators=NUMERIC_OPS,
        example_values=[500.0, 5000.0, 30000.0],
        applicable_brands=["corporate"],
        unit="INR",
        source_table="silver_corp_customer_attributes",
    ),
    AttributeDefinition(
        key="nbl.spend_per_bill",
        label="NBL Spend per Bill",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.FLOAT,
        description="Average spend per bill at Nature's Basket",
        operators=NUMERIC_OPS,
        example_values=[500.0, 1500.0, 3000.0],
        applicable_brands=["corporate"],
        unit="INR",
        is_computed=True,
        source_table="silver_corp_customer_attributes",
    ...
    AttributeDefinition(
        key="nbl.recency_days",
        label="NBL Recency (Days)",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Days since last Nature's Basket transaction",
        operators=NUMERIC_OPS,
        example_values=[5, 30, 90, 365],
        applicable_brands=["corporate"],
        unit="days",
    ...
    AttributeDefinition(
        key="nbl.last_bill_date",
        label="NBL Last Bill Date",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.DATE,
        description="Date of last Nature's Basket transaction",
        operators=DATE_OPS,
        example_values=["2025-12-20", "2026-01-05"],
        applicable_brands=["corporate"],
        source_table="silver_corp_customer_attributes",
    ...
    AttributeDefinition(
        key="nbl.l1_segment",
        label="NBL L1 Segment",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="NBL value segment (HVHF / LVHF / HVLF / LVLF)",
        operators=STRING_OPS,
        example_values=["HVHF", "LVHF", "HVLF", "LVLF"],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    AttributeDefinition(
        key="nbl.l2_segment",
        label="NBL L2 Segment",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="NBL behavioral segment (STAR / LOYAL / Win Back / New / ACTIVE / Inactive / LAPSER / Deep Lapsed)",
        operators=STRING_OPS,
        example_values=["STAR", "LOYAL", "Win Back", "New", "ACTIVE", "Inactive", "LAPSER", "Deep Lapsed"],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    AttributeDefinition(
        ...
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="City from the customer's NBL profile",
        operators=STRING_OPS,
        example_values=["Mumbai", "Pune", "Bengaluru"],
        applicable_brands=["corporate"],
        source_table="silver_corp_customer_attributes",
    ),
    AttributeDefinition(
        key="nbl.channel_presence",
        label="NBL Channel Presence",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.STRING,
        description="NBL channel footprint: Online / Offline / Omni",
        operators=STRING_OPS,
        example_values=["Online", "Offline", "Omni"],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    AttributeDefinition(
        key="nbl.spend_decile",
        label="NBL Spend Decile",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Spend decile within NBL (1=lowest, 10=top)",
        operators=NUMERIC_OPS,
        example_values=[1, 3, 5, 7, 10],
        applicable_brands=["corporate"],
        is_computed=True,
    ...
    AttributeDefinition(
        key="nbl.total_visits",
        label="NBL Total Visits",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Total distinct visits at Nature's Basket",
        operators=NUMERIC_OPS,
        example_values=[1, 5, 20],
        applicable_brands=["corporate"],
        unit="count",
    ...
    AttributeDefinition(
        key="spn.total_visits",
        label="SPN Total Visits",
        category=AttributeCategory.CROSS_BRAND,
        data_type=AttributeDataType.INTEGER,
        description="Total distinct visits at Spencers",
        operators=NUMERIC_OPS,
        example_values=[1, 5, 20, 100],
        applicable_brands=["corporate"],
        unit="count",
